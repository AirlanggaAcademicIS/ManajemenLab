...
class Penjadwalan_lk4 extends CI_Controller
{
...
function index($year = null, $month = null, $day = null, $idlabkom = 4){
	...
	function detail_event(){		
		...
		if ($this->form_validation->run() == FALSE){
			...
			$data = $this->penjadwalan->getEvent($this->input->post('year', true), $this->input->post('mon', true), $this->input->post('day', true), 4);
			if($data == null){
	...
	function add_event(){
		...
		$this->load->view('penjadwalan/add_event', $data);
	}
	...
	function delete_event(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			$rows = $this->penjadwalan->deleteEvent($this->input->post('year', true),$this->input->post('mon', true),$this->input->post('day', true), $this->input->post('del', true));
			if($rows > 0){
...
function detail($year = null, $month = null, $day = null, $idlabkom = 4 ){
		$year  = (empty($year) || !is_numeric($year))?  date('Y') :  $year;
		$month = (is_numeric($month) &&  $month > 0 && $month < 13)? $month : date('m');
		$day   = (is_numeric($day) &&  $day > 0 && $day < 31)?  $day : date('d');
	...
	function _setting(){
		return array(
			'start_day' 		=> 'monday',
			'show_next_prev' 	=> true,
			'next_prev_url' 	=> base_url('penjadwalan_lk4/index'),
			'month_type'   		=> 'long',
            'day_type'     		=> 'short',
